...
class UnrolledDenoising(BaseModel, ABC):
    ...
    def load_data(self, show_data=False):
        ...
        gen_data_train = real_data_train + self.noise_level * np.random.normal(size=real_data_train.shape)
        gen_data_test = real_data_test + self.noise_level * np.random.normal(size=real_data_test.shape)
    ...
    def build(self):
        ...
        def _create_model():
            ...
            Y = tf.keras.layers.Input((2500, 1), dtype=tf.float32)
            X_, Y_ = X, Y
            x_recs = []
    ...
    def train(self, steps):
        ...
        for step in tf.range(steps):
            ...
            with tf.GradientTape() as tape:
                out_batch, _ = self.model([gen_batch, gen_batch])
