...
class UnrolledOptimization(BaseModel, ABC):
    ...
    def build(self):
        ...
        def _unrolled_admm():
            ...
            S = tf.keras.layers.Input((2500, 1), dtype=tf.float32)
            Y = tf.keras.layers.Input((1560, 1), dtype=tf.float32)
            X_, S_, Y_ = X, S, Y
            x_recs = []
            for iter in range(0, self.num_iters):
                X_, S_, Y_ = ADMML1Layer(self.A, self.eta)([X_, S_, Y_])
                x_recs.append(X_)
            model = tf.keras.Model(inputs=[X, S, Y], outputs=[X_, S_, Y_])
            return model

        self.model = _unrolled_admm()
    ...
    def train(self, steps):
        ...
        for step in tf.range(steps):
            ...
            with tf.GradientTape() as tape:
                out_batch, _, _ = self.model([gen_batch, gen_batch, measurement_batch])
